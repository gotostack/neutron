...
def _generate_mangle_restore_dump(iptables_args):
    return ('# Generated by iptables_manager\n'
            ...
            '-A %(bn)s-PREROUTING -j %(bn)s-scope\n'
            '-A %(bn)s-PREROUTING -m connmark ! --mark 0x0/0xffff0000 '
            '-j CONNMARK --restore-mark '
            '--nfmask 0xffff0000 --ctmask 0xffff0000\n'
            '-A %(bn)s-PREROUTING -j %(bn)s-floatingip\n'
            '-A %(bn)s-float-snat -m connmark --mark 0x0/0xffff0000 '
            '-j CONNMARK --save-mark '
            '--nfmask 0xffff0000 --ctmask 0xffff0000\n'
            'COMMIT\n'
            '# Completed by iptables_manager\n' % iptables_args)
