...
# under the License.
"""Tests for neutron/services/metering/publisher/utils.py
"""
import datetime
...
class TestSignature(base.BaseTestCase):
    ...
    def test_meter_message_from_sample_with_metadata(self):
        self._test_meter_message_from_sample(False)

    def test_meter_message_from_sample_without_metadata(self):
        self._test_meter_message_from_sample(True)

    def _test_meter_message_from_sample(self, shrink_metadata):
        metadata = {'name': 'TestPublish'}
        sample = {'id': 'test01', 'name': 'test',
                  'type': 'cumulative', 'unit': '',
                  'volume': 1, 'user_id': 'test',
                  'project_id': 'test',
                  'resource_id': 'test_run_tasks',
                  'timestamp': datetime.datetime.utcnow().isoformat(),
                  'resource_metadata': metadata,
                  'source': 'openstack'}
        msg = utils.meter_message_from_sample(sample, shrink_metadata)
        self.assertEqual(1, msg['counter_volume'])
        if shrink_metadata:
            self.assertEqual({}, msg['resource_metadata'])
        else:
            self.assertEqual(metadata, msg['resource_metadata'])
